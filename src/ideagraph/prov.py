...
<https://www.w3.org/TR/prov-dm/>`_. Exporting to it lets ideagraph provenance be
consumed by the wider provenance ecosystem (validators, visualisers, stores).

The mapping is:

* :class:`~ideagraph.core.claim.Claim` and
  :class:`~ideagraph.core.evidence.Evidence` nodes -> ``prov:Entity``
* :class:`~ideagraph.core.activity.Activity` nodes -> ``prov:Activity``
* edge endpoints typed :attr:`~ideagraph.core.provenance.NodeType.ARTEFACT`
  -> ``prov:Entity`` stubs, and :attr:`~ideagraph.core.provenance.NodeType.AGENT`
  -> ``prov:Agent`` stubs (they have no dedicated node model yet)
* edges map to the PROV relation that matches their predicate; the ideagraph-only
...
def _qn(identifier: str) -> str:
    ...
    Args:
        identifier: A ideagraph node or edge id.
...
# ideagraph predicate it represents. ``wasInfluencedBy`` is handled separately
...
def _node_type_map(entities: dict[str, Any], activities: dict[str, Any], agents: dict[str, Any]) -> dict[str, NodeType]:
    """Map every qualified name in a document to its ideagraph NodeType.
...
def from_prov(document: dict[str, Any]) -> ProvenanceGraph:
    """Reconstruct a provenance graph from a ideagraph PROV-JSON document.

    This is the inbound counterpart to :func:`to_prov`. Because PROV-JSON is a
    narrower model than ideagraph's, the reconstruction is best-effort: fields
    ideagraph does not export (claim tags and timestamps, evidence timestamps,
