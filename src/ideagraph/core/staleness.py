...
Every piece of :class:`~ideagraph.core.evidence.Evidence` may carry a ``digest``:
...
:class:`~ideagraph.core.graph.ProvenanceGraph` to report — and optionally mark —
...
#: is compared verbatim against :attr:`~ideagraph.core.evidence.Evidence.digest`.
...
def mark_stale_claims(graph: ProvenanceGraph, resolver: DigestResolver) -> list[Claim]:
    ...
    Only claims currently marked :attr:`~ideagraph.core.claim.ClaimStatus.VALID`
    ...
    Returns:
        ...
        :attr:`~ideagraph.core.claim.ClaimStatus.STALE`.
